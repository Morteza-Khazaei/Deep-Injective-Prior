...
class upsqueeze(keras.layers.Layer):
...
class invertible_1x1_conv(keras.layers.Layer):
    ...
    def call(self, x, reverse=False, training=True):
        ...
        if not reverse:
            ...
            # print(w.shape, x.shape, w.dtype, x.dtype)
...
class conv_stack(keras.layers.Layer):
...
class affine_coupling(keras.layers.Layer):
...
class revnet_step(keras.layers.Layer):
...
class revnet(keras.layers.Layer):
